...
export class UploaderService {
  ...
  // método upload v2 para evitar el problema de los caractres.
  uploadV2(file: File, namefile: string) {
    const formData: FormData = new FormData();
    formData.append('fileKey', file, namefile);
    const req = new HttpRequest('POST', '/api/upload/file', formData, {
      reportProgress: true
    });
  ...
  private handleError(file: File) {
    ...
    return (error: HttpErrorResponse) => {
      ...
      this.showProgress(`${userMessage} ${message}`);
